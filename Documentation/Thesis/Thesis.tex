...
In collaboration with the \ac{ANT} research group at Saxion, this project focused on solving the operational bottlenecks of the existing \ac{QIUP} setup. By integrating hardware control and implementing an optimized \ac{FFT} algorithm in Python, the system successfully achieved automated, real-time extraction of phase, visibility and contrast images. 

\vspace{1cm}
\begin{footnotesize}
\textit{Disclaimer: Generative AI tools were used to assist in the writing process of this report, including grammar checking and language enhancement. In Addition to this, AI helped during research and literature review. All sources and references have been properly reviewed and cited.}
\end{footnotesize}
...
Moreover, custom samples were designed and fabricated using adobe illustrator for proper demonstration of the technology.
...
\section{Analysis and Specifications}
    ...
    \subsubsection{Nonlinear Interferometry} 
        ...
        \label{sec:nonlinear_interferometer}
    ...
    \subsubsection{Spontaneous Parametric Down-Conversion (SPDC)}
        ...
        When a pump laser illuminates a specialized nonlinear crystal (in this setup, a \ac{PPLN} crystal), pump photons spontaneously split into pairs of lower-energy photons, as shown in Figure \ref{fig:demonstrator}b the inital light beam splits only after going through the nonlinear crystal \cite{NonlinearInterferometry}. These newly created photons are designated as the signal and the idler. For this demonstrator, the signal photon lies within the visible spectrum and is detected by a standard \ac{CMOS} camera, whereas the idler photon is generated in the infrared spectrum and probes the sample.
        \label{sec:SPDC}
    ...
    \subsubsection{Indistinguishability and Interference}
        ...
        The schematic in Figure \ref{fig:demonstrator}b shows that the photons of each wavelength pass through the nonlinear crystal twice, once before and once after reflection. This creates a superposition state where the origin of the detected signal photon is fundamentally indistinguishable, leading to quantum interference effects that are crucial for imaging \cite{NonlinearInterferometry}.
...
\subsection{Hardware Control and Integration}
    ...
    \subsection{Image Processing Pipeline}
        ...
        \begin{enumerate}
            ...
            \item \textbf{Optional Spatial Filtering:} To suppress read noise, a user-configurable \ac{MA} filter is applied:
            \begin{verbatim}
...
\subsection{Interference Pattern Validation}
    ...
    \begin{figure}[H]
        ...
        \begin{subfigure}[b]{0.48\linewidth}
            ...
            \caption{Sinusoidal oscillation with \ac{MA} filtering.}
            \label{fig:real_filtered}
        \end{subfigure}
        
        \caption{Comparative analysis of the interference signal quality, demonstrating the noise reduction achieved through spatial \ac{MA} filtering.}
        \label{fig:oscillation_comparison}
    \end{figure}

    As evident in Figure \ref{fig:real_unfiltered}, the raw intensity curve is more erratic and impacted by sensor noise. Conversely, the filtered data (Figure \ref{fig:real_filtered}) demonstrates a smoother sinusoidal oscillation. This successful extraction confirms two critical system capabilities: first, that the automated hardware synchronization reliably resolves the physical interference pattern, and second, that the spatial \ac{MA} filter significantly enhances the \ac{SNR}, providing a highly stabilized data stack for the subsequent \ac{FFT} pipeline.
    ...
    \subsection{\texttt{Numpy.fft} vs. \texttt{pyFFTW} Performance Comparison}

    To optimize the performance of the \ac{FFT} processing pipeline, a direct comparison was conducted between the standard \texttt{numpy.fft} module and the highly optimized \texttt{pyFFTW} library. Both implementations were benchmarked using an identical dataset comprising a 32-frame stack at a resolution of $1440 \times 1080$ pixels.

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\linewidth]{"images/QIUP-tests/fft_time_taken/numpy_vs_fftw.png"}
        \caption{Comparison of execution times between \texttt{numpy.fft} and \texttt{pyFFTW}.}
        \label{fig:fft_time_comparison}
    \end{figure}

    As illustrated in Figure \ref{fig:fft_time_comparison}, the \texttt{numpy.fft} implementation required approximately 0.8 seconds to process the 32-frame stack. In contrast, the \texttt{pyFFTW} implementation exhibited a significant initial overhead, taking approximately 9.0 seconds to compute the optimal hardware-specific execution plan and process the first dataset. However, because this \ac{FFTW} plan is generated only once and cached for subsequent executions, the overhead is strictly a one-time initialization cost. Following the initial plan generation, \texttt{pyFFTW} processes the same 32-frame stack in approximately 0.077 seconds. This represents a greater than tenfold increase in processing speed over \texttt{numpy.fft}. This computational efficiency is critical for real-time visibility and phase reconstruction.
    ...
    At the beginning of the project, there was a theoretical concern that the hysteresis of the piezoelectric stage in open-loop mode would cause uneven spatial phase steps. This could potentially degrade the quality of the \ac{FFT} output and, hence the calculated quantum maps. To address this, both open-loop and closed-loop control modes were implemented and tested using the fabricated samples.
        ...
        \begin{figure}[H] 
            ...
            % Main Caption
            \caption{Comparison of Visibility, Contrast, and Phase reconstructions for the PSI sample using an open-loop phase scan without spatial \ac{MA} (no \ac{MA}) filtering. The rows demonstrate the raw effect of increasing the number of phase-shifted frames ($N=3, 8, 16, 32$) on the \ac{FFT} signal-to-noise ratio.}
        ...
        \begin{figure}[H] 
            ...
            % Main Caption
            \caption{Comparison of Visibility, Contrast, and Phase reconstructions for the PSI sample using an open-loop phase scan with \ac{MA} filtering. The rows demonstrate the effect of increasing the number of phase-shifted frames ($N=3, 8, 16, 32$) on the \ac{FFT} signal-to-noise ratio and image fidelity.}
            \label{fig:open_loop_psi_ma_grid}
        \end{figure}
        As observed in the acquired maps, the open-loop control mode successfully produces high-fidelity quantum images, clearly resolving visibility, contrast, and phase, despite the inherent hysteresis of the piezoelectric crystal. The reconstructions remain remarkably consistent across varying frame counts, and the application of the spatial \ac{MA} filter yields a significant enhancement in overall clarity. Crucially, the system's robustness against non-linear phase steps is enabled by the dynamic $F_1$ bin auto-detection algorithm detailed earlier. By computationally isolating the dominant interference frequency rather than relying on a strictly defined Fourier bin, the software effectively compensates for slight positional variations during the sweep. This algorithmic resilience actively mitigates spectral leakage, ensuring that the theoretical drawbacks of open-loop hysteresis do not practically degrade the final quantum measurements.

        \subsubsection{Closed-loop imaging}
        In contrast to the open-loop mode, closed-loop imaging actively engages the hardware PID servo and utilizes the strain gauge feedback to guarantee strictly equal spatial phase steps. The reconstructed visibility, contrast, and phase maps for this mode are presented in Figures \ref{fig:closed_loop_cat_noma_grid}, \ref{fig:closed_loop_cat_ma_grid}, \ref{fig:closed_loop_psi_noma_grid}, and \ref{fig:closed_loop_psi_ma_grid}. As observed in the open-loop tests, increasing the acquisition frame count progressively enhances the signal-to-noise ratio, and the application of the spatial \ac{MA} filter significantly minimizes background artifacts to yield highly defined edges,  
    ...
    \begin{figure}[p] 
        ...
        \caption{PSI sample: Closed-loop phase scan without spatial \ac{MA} (no \ac{MA}) filtering ($N=3, 8, 16, 32$).}
    ...
    \begin{figure}[p] 
        ...
        \caption{PSI sample: Closed-loop phase scan with \ac{MA} filtering ($N=3, 8, 16, 32$).}
        \label{fig:closed_loop_psi_ma_grid}
    \end{figure}

\newpage
\section{Conclusions and Recommendations}
This chapter summarizes the findings of the project, evaluating the system's performance against the initial objectives and requirements. Furthermore, it provides strategic recommendations for the future development of the demonstrator.

\subsection{Conclusions}
This internship project successfully transformed the \ac{QIUP} laboratory setup into a robust, automated, and user-friendly demonstrator platform. A significant achievement of this work is the development of a unified software architecture that integrates the \ac{CMOS} camera and piezoelectric stage control into a single, cohesive application. By leveraging the Thorlabs \ac{TSI} \ac{SDK} and the Kinesis .NET API, the system streamlines hardware communication, effectively replacing manual, multi-tool workflows with a centralized automation system.

To address the project's requirements, two distinct versions of the control software were developed: one operating in an open-loop mode and the other in a closed-loop configuration. While the closed-loop mode was initially pursued to address theoretical concerns regarding piezoelectric hysteresis, tested results demonstrated that the hysteresis effect was negligible over the short scanning distances required. Consequently, the open-loop version proved to be the more stable, faster, and highly responsive solution, making it the superior choice for real-time live demonstrations.

The project exceeded its original scope by implementing a feature-rich \ac{GUI} that includes advanced functionality such as spatial \ac{MA} filtering, real-time pixel-wise intensity monitoring, and dynamic colormap scaling. Furthermore, by integrating highly optimized libraries such as \ac{FFTW}, the system achieves the low-latency processing required to render visibility, contrast, and phase maps in real-time. Ultimately, this unified system proves that complex quantum optical setups can be managed via intuitive, single-application software, making \ac{QIUP} technology significantly more accessible for research and educational outreach.

\subsection{Recommendations}
Building upon the success of the current \ac{QIUP} demonstrator, the following recommendations are proposed to evolve the current system:

\begin{enumerate}
    \item \textbf{Transition to an Analytical Platform:} The current \ac{GUI} is optimized for demonstration and clarity. It is recommended to extend the interface to function as an analytical suite. This would involve displaying more quantitative information, such as saving and displaying \ac{FFT} results. Additionally, it is possible to render a detected sample in 3D by applying the Fourier phase information to a height map, which could be visualized using libraries such as Matplotlib or Plotly. This would provide a more comprehensive tool for researchers to analyze and interpret quantum imaging data directly within the application.
    
    \item \textbf{Integration of AI-Driven Analysis:} As artificial intelligence becomes increasingly integral to future scientific developments, it is recommended to implement an AI agent to analyze the calculated visibility, contrast, and phase images. By calling a secure API key (e.g., Anthropic’s Claude models), an AI agent could be created specifically to interpret these maps, potentially identifying patterns or extracting information beyond the reach of traditional Fourier analysis. This would bridge the gap between complex quantum imaging data and the understanding of underlying physical principles.
    
    \item \textbf{Development of IR-Transparent Samples:} To further highlight the unique capabilities of the \ac{QIUP} system, it is recommended to fabricate a sample that appears opaque to the naked eye but is fully transparent at the 1550 nm infrared frequency. Such a sample would provide a dramatic and tangible demonstration of the system’s ability to "see" through materials, providing an even more compelling showcase of the underlying quantum physical principles than the current opaque-mask approach.

    \item \textbf{Development of gas sensing capabilities:} The current system is optimized for imaging solid samples. However, the underlying principles of quantum interference could also be applied to sensing applications, such as detecting trace gases or monitoring environmental conditions. By modifying the sample chamber to allow for gas flow and integrating appropriate sensors, the system could be adapted to perform sensitive measurements of gas concentrations based on changes in the interference pattern, opening up new avenues for research and practical applications in fields such as environmental monitoring and industrial process control.
\end{enumerate}
