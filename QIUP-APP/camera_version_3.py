...
class CameraController:
    ...
    def _add_scale_bar(self, image, scale_length_px):
        """Draws a hardcoded white scale bar perfectly centered at the bottom."""
        h, w = image.shape[:2]
        
        # Calculate horizontal center
        center_x = w // 2
        half_length = scale_length_px // 2
        
        # Position: 30 pixels padding from bottom, expanding from center
        x_start = center_x - half_length
        x_end = center_x + half_length
        y_pos = h - 30
        
        # Draw the scale line
        cv2.line(image, (x_start, y_pos), (x_end, y_pos), (255, 255, 255), 4)
        
        # Add the "1 mm" label exactly centered above the line
